...
let Pgrass = require("./modules/Pgrass");
...
function createMatrix(size, grass, grasseater, predator, human, bomb, pgrass) {
  ...
  for (var y = 0; y < matrix.length; y++) {
    for (var x = 0; x < matrix[y].length; x++) {
      if (matrix[y][x] == 1) {
        ...
        let pGrass = new Pgrass(x, y);
        pGrassArr.push(pGrass);
      }
    }
  }
}
...
function game() {
  ...
  let sendData = {
    ...
    pGrassCounter: pGrassArr.length,
  };
...
function getStatistics() {
  ...
  statistics.pGrass = pGrassArr.length;
}
